...
class MongoDBBookStore:
    def __init__(self):
        ...
        self.db = self.client['BookStore_Crawler_Full'] #Create Database      
        pass
    
    def process_item(self, item, spider):
        
        collection =self.db['BookList'] #Create Collection or Table
        try:
    ...
    def process_item(self, item, spider):
        ...
        if item:
            with open('bookdata.csv', 'a', encoding='utf-8', newline='') as file:
                ...
                if not file_exists:
                    writer.writerow([
                        'Ten_Sach', 'SKU', 'The_Loai', 'Gia_Moi','Gia_Cu','Giam_Gia','Rating','Review_Count', 'Ten_NCC',
